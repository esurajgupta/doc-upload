<template>
    <div class="border px-16 py-12 rounded bg-sky-50/50" @click="onClickSimulation()" style="cursor: pointer;">
        <div class="flex justify-center items-center flex-col">
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="50" height="50" viewBox="0 0 50 50"
                style="fill: #5D9FEC;opacity:0.5;">
                <path
                    d="M 5 4 C 3.346 4 2 5.346 2 7 L 2 13 L 3 13 L 47 13 L 48 13 L 48 11 C 48 9.346 46.654 8 45 8 L 18.044922 8.0058594 C 17.765922 7.9048594 17.188906 6.9861875 16.878906 6.4921875 C 16.111906 5.2681875 15.317 4 14 4 L 5 4 z M 3 15 C 2.448 15 2 15.448 2 16 L 2 43 C 2 44.657 3.343 46 5 46 L 45 46 C 46.657 46 48 44.657 48 43 L 48 16 C 48 15.448 47.552 15 47 15 L 3 15 z">
                </path>
            </svg>
            <p class="font-medium text-md text-slate-700">Drag your files here</p>
        </div>
    </div>
    <h2 class="second-border"><span>or</span></h2>
    <div class="flex w-100 justify-center items-center">
        <p class="underline underline-offset-1 text-custom font-semibold" @click="onClickSimulation()"
            style="cursor: pointer;">Browse Your Computer</p>
    </div>
    <input id="getFile" type="file" style="display: none;" @input="event => onClickInput(event)" />
</template>

<script>
export default {
    emits: ['uploadDocument'],
    methods: {
        onClickInput(e) {
            console.log(e.target.files[0]);
            this.$emit('uploadDocument', e.target.files[0]);
        },
        onClickSimulation() {
            const id = document.getElementById("getFile");
            id.click();
        }
    }
}
</script>

<style scoped>
.border {
    border: 1px dashed #d3e4f8;
}

.text-custom {
    color: #5D9FEC;
}

h2 {
    width: 100%;
    text-align: center;
    border-bottom: 2px solid #d3e4f8;
    line-height: 0.1em;
    margin: 15px 0 20px;
}

h2 span {
    background: #fff;
    padding: 0 10px;
}
</style>
